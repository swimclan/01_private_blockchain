...
class Blockchain {
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const elapsed = currentTime - msgTime;
      const timeIsValid = elapsed < 300;
      const validSigned = bitcoinMessage.verify(message, address, signature);
      console.log(timeIsValid, validSigned);
      if (!timeIsValid || !validSigned) {
  ...
  getBlockByHash(hash) {
    ...
    return new Promise((resolve, reject) => {
      ...
      return reject("No block found with that hash");
    });
  }
  ...
  getStarsByWalletAddress(address) {
    ...
    return new Promise(async (resolve, reject) => {
      for (const block of self.chain) {
        let decodedBlock;
        try {
          decodedBlock = await block.getBData();
        } catch (err) {
          return reject(err);
        }

        if (decodedBlock.owner === address) {
